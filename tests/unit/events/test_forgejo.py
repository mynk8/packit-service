...
from packit_service.events import event
import pytest
...
from packit_service.events import event_data
from packit_service.events.github.push import Commit
...
def test_parse_forgejo_tag_push(tag_push):
    ...
    assert event_object.project_url == "https://v10.next.forgejo.org/packit/test-repo-to-generate-events"
...
def test_parse_pull_request(pull_request_opened):
    ...
    mock_project = flexmock(full_repo_name="packit/test-repo-to-generate-events", service=mock_service)
    flexmock(ServiceConfig).should_receive("get_project").and_return(mock_project)
...
def test_parse_forgejo_pr_vs_issue_comment_discrimination(pull_request_comment, issue_comment):
    ...
    assert pr_comment_event.project_url == "https://v10.next.forgejo.org/packit/test-repo-to-generate-events"
